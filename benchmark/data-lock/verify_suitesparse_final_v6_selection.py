...
V6_PAYLOAD_PREFIX = "benchmark/suitesparse/final-heldout-v6/"
    ...
    with path.open(newline="") as source:
        for row in csv.reader(source, delimiter="\t"):
            if (
                row
                and row[0] == "matrix"
                and not row[1].startswith(V6_PAYLOAD_PREFIX)
            ):
                result.add(row[2])
    return result
...
def read_frozen(path: Path) -> list[dict[str, object]]:
    ...
    for values in rows[1:]:
        if len(values) != len(header):
            raise ValueError("v6 selection row width changed")
...
def main() -> int:
    ...
    if frozen != expected:
        raise ValueError(f"v6 deterministic selection mismatch: {frozen!r} != {expected!r}")
    print("SMAVE_SUITESPARSE_FINAL_V6_SELECTION 1")
    return 0
