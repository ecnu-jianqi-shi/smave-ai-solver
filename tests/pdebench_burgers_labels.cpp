...
std::vector<double> solve_cyclic_tridiagonal(
    ...
    diagonal.back() -= top_right * bottom_left / gamma;
    auto primary = solve_tridiagonal(lower, diagonal, upper, right_hand_side);
    std::vector<double> update(diagonal.size());
    update.front() = gamma;
    update.back() = bottom_left;
    const auto correction = solve_tridiagonal(lower, diagonal, upper, update);
    const auto factor =
        (primary.front() + top_right * primary.back() / gamma) /
...
std::vector<double> solve_frozen_burgers(
...
double relative_residual(
    ...
    for (std::size_t index = 0; index < state.size(); ++index) {
        const auto previous = index == 0 ? state.size() - 1 : index - 1;
        const auto next = index + 1 == state.size() ? 0 : index + 1;
        const auto convection = convection_scale * state[index];
        const auto product = (1.0 + 2.0 * diffusion_number) * solution[index] +
            (-diffusion_number - convection) * solution[previous] +
            (-diffusion_number + convection) * solution[next];
        const auto residual = state[index] - product;
        residual_squared += residual * residual;
        right_hand_side_squared += state[index] * state[index];
    }
...
void write_tensor(
    ...
    if (!output) throw std::runtime_error("cannot write Burgers solver labels");
}
    ...
    try {
        if (argc != 6) {
            throw std::invalid_argument(
                "usage: smave_pdebench_burgers_labels FILE OUTPUT_PREFIX "
        ...
        if (H5Sget_simple_extent_dims(tensor_space.get(), dimensions, nullptr) != 3 ||
            ...
            requested_steps >= dimensions[1] || dimensions[2] < 3) {
            throw std::invalid_argument("solver-label split exceeds Burgers tensor shape");
        }
        ...
        const double delta_x = 1.0 / static_cast<double>(width);
        const double diffusion_number =
            viscosity * delta_time / (delta_x * delta_x);
        ...
        for (std::size_t sample_offset = 0; sample_offset < sample_count;
            ...
            for (std::size_t step = 0; step < requested_steps; ++step) {
                auto solution = solve_frozen_burgers(
                    state, diffusion_number, convection_scale);
                maximum_residual = std::max(maximum_residual,
                    relative_residual(
                        state, solution, diffusion_number, convection_scale));
        ...
        if (!std::isfinite(maximum_residual) || maximum_residual > 1.0e-12) {
            throw std::runtime_error(
                "generated Burgers label failed original FP64 residual gate");
        }
        ...
        manifest << smave::kPdebenchTrainingSetSchemaVersion << '\n'
                 ...
                 << std::setfill('0') << hash << "\"\nEND\n";
        if (!manifest) throw std::runtime_error("cannot write Burgers manifest");
        manifest.close();
        ...
        std::cout << "PDEBench Burgers solver labels="
                  << sample_count * requested_steps
                  << " width=" << width
                  << " maximum_residual=" << maximum_residual << '\n';
        return 0;
    } catch (const std::exception& error) {
        std::cerr << "PDEBench Burgers solver-label export failed: "
                  << error.what() << '\n';
        return 2;
    }
}
