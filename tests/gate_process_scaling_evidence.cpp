...
std::pair<double, double> bootstrap_interval(
    ...
    medians.reserve(bootstrap_resamples);
    std::vector<double> draw(values.size());
    for (std::size_t iteration = 0; iteration < bootstrap_resamples; ++iteration) {
...
std::vector<Values> linear_values(
    ...
    for (const auto& path : scenario_paths) {
        ...
        if (!outcome.success) {
            throw std::runtime_error("linear process scaling fixture solve failed");
        }
...
struct ProcessSummary {
    std::uint64_t decision_mismatches{};
    std::uint64_t residual_mismatches{};
};
...
void write_all(int descriptor, const void* data, std::size_t size) {
    ...
    while (size != 0) {
        ...
        if (written <= 0) throw std::runtime_error("process scaling pipe write failed");
        bytes += written;
...
void read_all(int descriptor, void* data, std::size_t size) {
    ...
    while (size != 0) {
        ...
        if (received <= 0) throw std::runtime_error("process scaling pipe read failed");
        bytes += received;
        size -= static_cast<std::size_t>(received);
    }
}

ProcessSummary run_child(
    const smave::Runtime& runtime,
    const smave::BlockIR& block,
    const std::vector<Values>& values,
    const std::vector<smave::GateResult>& authority,
    std::size_t worker,
    std::size_t worker_count) {
    ProcessSummary summary;
    for (std::size_t index = worker; index < values.size(); index += worker_count) {
        const auto observed = runtime.evaluate_gate_fused(block, values[index], true);
        if (observed.decision != authority[index].decision) {
            ++summary.decision_mismatches;
        }
        const double scale = std::max(
            {1.0, std::abs(observed.residual_inf),
             std::abs(authority[index].residual_inf)});
        if (std::abs(observed.residual_inf - authority[index].residual_inf) >
            1.0e-15 * scale) {
            ++summary.residual_mismatches;
        }
    }
    return summary;
}
...
double run_processes(
    ...
    std::vector<std::array<int, 2>> pipes(worker_count);
    std::vector<pid_t> children;
    children.reserve(worker_count);
    const auto started = std::chrono::steady_clock::now();
    for (std::size_t worker = 0; worker < worker_count; ++worker) {
        if (::pipe(pipes[worker].data()) != 0) {
            throw std::runtime_error("process scaling pipe creation failed");
        }
        const pid_t child = ::fork();
        if (child < 0) throw std::runtime_error("process scaling fork failed");
        if (child == 0) {
            ::close(pipes[worker][0]);
            try {
                const auto summary = run_child(
                    runtime, block, values, authority, worker, worker_count);
                write_all(pipes[worker][1], &summary, sizeof(summary));
    ...
    for (std::size_t worker = 0; worker < worker_count; ++worker) {
        ProcessSummary summary;
        read_all(pipes[worker][0], &summary, sizeof(summary));
        ::close(pipes[worker][0]);
        decision_mismatches += static_cast<std::size_t>(summary.decision_mismatches);
        residual_mismatches += static_cast<std::size_t>(summary.residual_mismatches);
    }
    for (const pid_t child : children) {
        int status = 0;
        if (::waitpid(child, &status, 0) != child || !WIFEXITED(status) ||
            WEXITSTATUS(status) != 0) {
            throw std::runtime_error("process scaling child failed");
        }
    }
...
ScalingResult measure(
    ...
    const auto run = [&](std::size_t worker_count) {
        return run_processes(
            runtime, block, values, authority, worker_count,
            result.decision_mismatches, result.residual_mismatches);
    };
    ...
    for (std::size_t index = 0; index < worker_counts.size(); ++index) {
        ...
        for (std::size_t repetition = 0; repetition < repetitions; ++repetition) {
            paired[repetition] = timings.front()[repetition] / timings[index][repetition];
        }
        result.paired_speedup[index] = median(paired);
        const auto interval = bootstrap_interval(
            paired, UINT64_C(20260724) + index * 100 + result.family.size());
...
void write_raw_samples(
    ...
    if (!output) throw std::runtime_error("cannot write process scaling raw samples");
    output << std::setprecision(17)
           << "SMAVE_GATE_PROCESS_SCALING_SAMPLES 1\n"
           << "contract=process-isolated-gate-worker-scaling-raw-samples\n"
...
void write_result(std::ostream& output, const ScalingResult& result) {
    ...
    output << result.family << ".decision_mismatches=" << result.decision_mismatches << '\n'
           << result.family << ".residual_mismatches=" << result.residual_mismatches << '\n';
}
    ...
    try {
        if (argc != 5) {
            throw std::invalid_argument(
                "usage: gate_process_scaling_evidence linear.mo scenarios nonlinear.mo output");
        }
        ...
        if (!output) throw std::runtime_error("cannot write process scaling evidence");
        output << std::setprecision(17)
        ...
        return std::cerr << "process gate scaling failure: " << error.what() << '\n', 2;
    }
}
