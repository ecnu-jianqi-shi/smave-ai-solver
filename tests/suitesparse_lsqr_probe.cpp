...
ResidualMetrics residual_metrics(
    ...
    for (std::size_t row = 0; row < system.size(); ++row) {
        metrics.residual_inf = std::max(
            metrics.residual_inf, std::abs(product[row] - system.right_hand_side[row]));
    ...
    try {
        if (argc != 4) {
            throw std::invalid_argument(
                "usage: suitesparse-lsqr-probe MATRIX_PATH MATRIX_INDEX MAX_ITERATIONS");
        }
        const auto matrix = smave::benchmark::read_matrix_market(argv[1]);
        const std::size_t matrix_index = std::stoull(argv[2]);
        const int maximum_iterations = std::stoi(argv[3]);
        if (maximum_iterations <= 0) {
            throw std::invalid_argument("MAX_ITERATIONS must be positive");
        }

        std::uint64_t seed = UINT64_C(0x5a17e5d3c92b4f81) + matrix_index * 8U;
        std::cout << std::setprecision(17)
                  << "kind\ttolerance\tconverged\tbreakdown\tstagnated"
                  << "\titerations\tresidual_inf\tbackward_error\tbest_residual_inf"
                  << "\treason\n";
        for (const std::string kind : {
            ...
            for (const double tolerance : {1.0e-6, 1.0e-10}) {
                ...
                const auto result = smave::least_squares_qr(
                    system, std::vector<double>(system.size()),
                    1.0e-12, tolerance, maximum_iterations);
                const auto metrics = result.solution.size() == system.size()
                    ? residual_metrics(system, result.solution)
                    : ResidualMetrics{
                          std::numeric_limits<double>::infinity(),
                          std::numeric_limits<double>::infinity()};
                const double best_residual = result.residual_history.empty()
                    ? std::numeric_limits<double>::infinity()
                    : *std::min_element(
                          result.residual_history.begin(), result.residual_history.end());
