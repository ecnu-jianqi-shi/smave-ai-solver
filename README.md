...
`reproduce-phase7` 与 ReleaseStore 回归现已通过：64 个验证场景的 DatasetManifest、Validation v3、10 轮 Operator Benchmark v3（shadow/canary `promotion_ready`、break-even gate）、HMAC-SHA256 签名、owner-only 密钥、激活/回滚和 bundle/dataset 篡改拒绝均验证成功。该发布链仅作为 legacy engineering regression，其中的签名、密钥、跨进程锁、恢复和回滚证据不进入求解器 claim surface。当前核心复现只要求 solver-related tests、数值证据检查与论文构建。
